...
func (m *mapImpl) loadMap() {
	...
	for i := 0; i < m.numRows; i++ {
		...
		for j := 0; j < m.numCols; j++ {
			if sm[i][j] == 'S' {
				m.startPoint = common.Point{Y: float32(i)*m.blockHeight + m.blockHeight/2, X: float32(j)*m.blockWidth + m.blockWidth/2}
				continue
			}
			if sm[i][j] == 'E' {
				m.endPoint = common.Point{Y: float32(i) * m.blockHeight, X: float32(j) * m.blockWidth}
				continue
			}
...
func (m *mapImpl) GetStartPoint() common.Point {
	return m.startPoint
}

func (m *mapImpl) GetEndPoint() common.Point {
	return m.endPoint
}
